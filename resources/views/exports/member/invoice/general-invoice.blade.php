...
<body>
    <div class="invoice-container">
        @php
            ...
            function safeDateFormat($date, $format = 'F d, Y') {
                if (empty($date)) {
                    return 'N/A';
                }
        ...
        <div class="invoice-header">
            <div class="company-info">
                ...
                <p>Generated: {{ $generated_at ?? date('Y-m-d H:i:s') }}</p>
            </div>

            <div class="invoice-title">
                <h2>INVOICE</h2>
                <p class="mb-10">Reference: <strong>{{ $invoice->reference ?? 'N/A' }}</strong></p>
                <p>Date: {{ safeDateFormat($invoice->invoice_date ?? now(), 'F d, Y') }}</p>
                <p>Due Date: {{ safeDateFormat($invoice->due_date ?? now(), 'F d, Y') }}</p>
            </div>
        </div>
            ...
            <div class="info-box">
                ...
                <div class="info-details">
                    ...
                    <div class="info-row">
                        ...
                        <span>{{ safeDateFormat($subscription->start_date ?? now(), 'M d, Y') }}</span>
                    </div>
                    @if (!empty($subscription->end_date))
                        <div class="info-row">
                            <span class="info-label">End Date:</span>
                            <span>{{ safeDateFormat($subscription->end_date, 'M d, Y') }}</span>
                        </div>
                    @endif
                    <div class="info-row">
            ...
            <div class="info-box">
                ...
                <div class="info-details">
                    ...
                    <div class="info-row">
                        ...
                        <span>{{ safeDateFormat($invoice->invoice_date ?? now(), 'M d, Y') }}</span>
                    </div>
                    <div class="info-row">
                        <span class="info-label">Due Date:</span>
                        <span>{{ safeDateFormat($invoice->due_date ?? now(), 'M d, Y') }}</span>
                    </div>
                    <div class="info-row">
                        <span class="info-label">Period:</span>
                        <span>{{ safeDateFormat($invoice->from_date ?? now(), 'M d, Y') }} to
                            {{ safeDateFormat($invoice->to_date ?? now(), 'M d, Y') }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        @if (($invoice->total_check_ins ?? 0) > 0)
            <div class="checkin-summary">
                ...
                <p>Check-in period: {{ safeDateFormat($invoice->from_date ?? now(), 'M d, Y') }} to
                    {{ safeDateFormat($invoice->to_date ?? now(), 'M d, Y') }}</p>
            </div>
        @endif
                ...
                <tbody>
                    <tr>
                        <td>
                            ...
                            Period: {{ safeDateFormat($invoice->from_date ?? now(), 'M d, Y') }} to
                            {{ safeDateFormat($invoice->to_date ?? now(), 'M d, Y') }}
                            @if (($invoice->total_check_ins ?? 0) > 0)
                    ...
                    <tbody>
                        @foreach ($transactions as $transaction)
                            ...
                            <tr>
                                ...
                                <td>{{ safeDateFormat($transactionDate, 'M d, Y') }}</td>
                                <td>{{ $paymentMethod }}</td>
                    ...
                    @php
                        try {
                            ...
                            if (!empty($invoice->invoice_date) && !empty($invoice->due_date)) {
                                ...
                                $daysDiff = $dueDate->diffInDays($invoiceDate);
                            }
                            echo $daysDiff;
                        } catch (Exception $e) {
                            echo '30';
